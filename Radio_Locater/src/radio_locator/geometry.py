...
def _circle_from_two(first: FloatArray, second: FloatArray) -> MinimumEnclosingCircle:
    """返回以两点为直径的圆。"""

    center = (first + second) / 2.0
    radius = float(np.linalg.norm(first - second)) / 2.0
    return MinimumEnclosingCircle(center, radius)
...
def _circle_from_three(
    ...
    edge_ac = third - first
    denominator = 2.0 * cross_2d(edge_ab, edge_ac)
    if abs(denominator) < 1e-15:
        ...
        return _circle_from_two(*longest)
    ab_squared = float(np.dot(edge_ab, edge_ab))
    ac_squared = float(np.dot(edge_ac, edge_ac))
    center = first + np.array(
        [
            -ac_squared * edge_ab[1] + ab_squared * edge_ac[1],
            ac_squared * edge_ab[0] - ab_squared * edge_ac[0],
        ],
        dtype=float,
    ) / denominator
...
def minimum_enclosing_circle(points: FloatArray) -> MinimumEnclosingCircle:
    """用随机增量法求点集的最小包围圆。

    输入点集会被复制并随机打乱；对凸多边形裁剪得到的数百顶点也能在线性期望时间内
    完成。递归深度上限设为输入点数的两倍，避免 Python 默认递归限制。
    """

    points = np.asarray(points, dtype=float)
    if len(points) == 0:
        raise ValueError("minimum enclosing circle is undefined for empty points")
    unique = np.unique(points, axis=0)
    if len(unique) == 1:
        return MinimumEnclosingCircle(unique[0].copy(), 0.0)

    shuffled = unique.tolist()
    random.shuffle(shuffled)
    previous_limit = sys.getrecursionlimit()
    sys.setrecursionlimit(max(previous_limit, len(shuffled) * 2 + 20))

    def contains(circle: MinimumEnclosingCircle, point: FloatArray) -> bool:
        return float(np.linalg.norm(point - circle.center)) <= circle.radius + 1e-9

    def welzl(boundary: list[FloatArray], remaining: list[FloatArray]) -> MinimumEnclosingCircle:
        if not remaining or len(boundary) == 3:
            if len(boundary) == 0:
                return MinimumEnclosingCircle(np.zeros(2, dtype=float), 0.0)
            if len(boundary) == 1:
                return MinimumEnclosingCircle(np.asarray(boundary[0], dtype=float), 0.0)
            if len(boundary) == 2:
                return _circle_from_two(
                    np.asarray(boundary[0], dtype=float),
                    np.asarray(boundary[1], dtype=float),
                )
            return _circle_from_three(
                np.asarray(boundary[0], dtype=float),
                np.asarray(boundary[1], dtype=float),
                np.asarray(boundary[2], dtype=float),
            )
        point = np.asarray(remaining.pop(), dtype=float)
        circle = welzl(boundary, remaining)
