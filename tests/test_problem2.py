...
from problems.problem2.plotting import plot_screening_principles
from problems.problem2.plotting import plot_first_detection_posterior_grid
from problems.problem2.plotting import plot_intersection_angle_constraints
...
def test_screening_principles_are_four_independent_dual_format_figures(
    tmp_path: Path,
) -> None:
    """四项粗筛原则应分别生成无共享画布的 PNG 和 PDF 文件。"""

    outputs = tuple(
        tmp_path / f"screening_principle_{index}.png"
        for index in range(1, 5)
    )
    plot_screening_principles(outputs)
    for output in outputs:
        assert output.stat().st_size > 5_000
        assert output.with_suffix(".pdf").stat().st_size > 5_000


def test_first_detection_posterior_grid_uses_dual_format_output(
    tmp_path: Path,
) -> None:
    """M1 后验离散采样图应基于真实网格并同时输出 PNG、PDF。"""

    measurement = BearingMeasurement((-900.0, -500.0), 31.363757)
    posterior = build_posterior_grid(measurement, grid_step_m=45.0)
    output = tmp_path / "m1_posterior_grid.png"
    plot_first_detection_posterior_grid(measurement, posterior, output)
    assert output.stat().st_size > 5_000
    assert output.with_suffix(".pdf").stat().st_size > 5_000


def test_intersection_angle_constraint_uses_dual_format_output(
    tmp_path: Path,
) -> None:
    """不同交会角的几何示意图也应生成 PNG 和 PDF。"""

    output = tmp_path / "intersection_angle_constraints.png"
    plot_intersection_angle_constraints(output)
    assert output.stat().st_size > 5_000
    assert output.with_suffix(".pdf").stat().st_size > 5_000
