...
class SelectionResult:
    ...
    @property
    def maximum_score(self) -> CandidateScore:
        """返回主模型细网格中的目标函数最大点，供增强绘图标注。"""
    ...
    @property
    def coarse_refinement_centers(self) -> tuple[CandidateScore, ...]:
        """暴露主模型采用的唯一粗网格细化中心。"""

        return (
            max(
                (score for score in self.coarse_scores if score.feasible),
...
def sample_selected_second_region_diameters(
    ...
    """生成增强直径分布图所需样本，不改变 master 分支的选点模型。"""
