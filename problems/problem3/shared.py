...
def route_aligned_clearance_point(
    circle: MinimumEnclosingCircle,
    current_position: tuple[float, float],
    following_position: tuple[float, float] | None,
    clearance_radius_m: float,
) -> tuple[float, float]:
    """在最小覆盖圆给出的安全余量内选择更顺路的清除点。

    若 ``|q-c| <= clearance_radius-rho``，由三角不等式可知圆内任意可能源位置
    到 ``q`` 的距离均不超过清除半径。因此这里只在该保证安全的子区域中优化，
    不依赖真值，也不引入清除失败风险。
    """

    center = np.asarray(circle.center, dtype=float)
    # 留出微小数值余量；正式清除阈值本身还使用19.8m而非题面20m。
    slack = max(float(clearance_radius_m - circle.radius) - 1e-6, 0.0)
    if slack <= 1e-12:
    ...
    if following_position is None:
        ...
        if distance <= slack:
            ...
            selected = center + slack * direction / distance
        else:
            selected = center
        return float(selected[0]), float(selected[1])

    following = np.asarray(following_position, dtype=float)
    segment = following - current
    squared_length = float(np.dot(segment, segment))
    if squared_length > 1e-18:
        ratio = float(np.dot(center - current, segment) / squared_length)
        ratio = min(max(ratio, 0.0), 1.0)
        projection = current + ratio * segment
    else:
        projection = current
    offset = projection - center
    offset_distance = float(np.linalg.norm(offset))
    if offset_distance <= slack:
        selected = projection
    elif offset_distance > 1e-12:
        # 圆外最近的线段投影方向是优良初值；再枚举边界，直接最小化断线长度。
        angles = np.linspace(0.0, 2.0 * math.pi, 721, endpoint=False)
        boundary = center + slack * np.column_stack(
            (np.cos(angles), np.sin(angles))
        )
        costs = np.linalg.norm(boundary - current, axis=1) + np.linalg.norm(
            boundary - following, axis=1
        )
        selected = boundary[int(np.argmin(costs))]
    else:
        selected = center
    return float(selected[0]), float(selected[1])
...
def localize_channel(
    ...
    *,
    ...
    route_following_point: tuple[float, float] | None = None,
    optimize_clear_point: bool = False,
) -> None:
    ...
    for _ in range(state.settings.localization_max_measurements):
        ...
        if (
            ...
            if not clear_when_ready:
                return
            center = route_aligned_clearance_point(
                track.clear_circle,
                state.position,
                route_following_point,
                clear_radius_m,
            ) if optimize_clear_point else (
