...
def plot_run_trajectory(
    ...
    cleared_count = sum(event.result == "clear_success" for event in events)
    source_total = str(len(sources)) if sources else "未知"
    virtual_time = max((event.virtual_time_s for event in events), default=0.0)
    ...
    axis.set_title(
        f"问题3单局运动复盘：{strategy}\n"
        f"虚拟耗时 {virtual_time:.3f} s；已清除/总数量 {cleared_count}/{source_total}"
    )
...
def plot_clearance_details(
    ...
    figure.suptitle(
        f"问题3清除位置特写：{strategy}；虚拟耗时 {virtual_time:.3f} s；"
        f"已清除/总数量 {cleared_count}/{len(sources)}",
        fontsize=13,
    )
