...
def configure_chinese_font() -> None:
    """选择 Linux 中实际存在的中文论文字体，并保证负号正常显示。"""
    ...
    selected = next((name for name in preferred if name in installed), "DejaVu Serif")
...
def style_figure_for_paper(figure: plt.Figure) -> None:
    ...
    if not any(artist.get_gid() == "paper-frame" for artist in figure.artists):
        frame = Rectangle(
            (0.004, 0.004),
            0.992,
            0.992,
