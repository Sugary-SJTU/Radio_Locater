...
from problems.problem4.coverage import arena_grid
...
class ChannelBelief:
    """不存在原子和 ``(网格位置,固定接收半径,类型,定向角)`` 上的联合分布。"""
    exists: NDArray[np.bool_]
    points: FloatArray
    radii_m: FloatArray
    source_type: NDArray[np.int8]
    direction_deg: FloatArray
    weights: FloatArray
    ...
    @classmethod
    def initialize(cls, settings: Problem4Settings, channel: int) -> "ChannelBelief":
        ...
        source_type = np.concatenate(
            (np.zeros(1, dtype=np.int8), rng.integers(0, 2, target).astype(np.int8))
        )
        direction_deg = np.concatenate(
            (np.zeros(1, dtype=float), rng.uniform(0.0, 360.0, target))
        )
        ...
        return cls(
            exists, points, radii, source_type, direction_deg, weights, resolution,
            np.ones(target+1, dtype=bool), settings.direction_bin_deg,
            settings.probability_prune_threshold, settings.arena_radius_m,
        )

    def copy(self) -> "ChannelBelief":
        """复制联合状态，供场景树虚拟观测。"""
        return ChannelBelief(
            self.exists.copy(), self.points.copy(), self.radii_m.copy(),
            self.source_type.copy(), self.direction_deg.copy(), self.weights.copy(),
            self.resolution_m.copy(), self.active.copy(), self.direction_bin_deg,
            self.prune_threshold, self.arena_radius_m,
        )
    ...
    def restrict_to_bearings(self, observations: list[tuple[tuple[float, float], float]]) -> None:
        ...
        for station, bearing in observations:
            ...
            compatible &= self._front_mask(station)
    ...
    def _front_mask(self, position: tuple[float, float]) -> NDArray[np.bool_]:
        """定向源要求检测点位于发射方向左右 90° 前向半平面内。"""

        vectors = self.points - np.asarray(position, dtype=float)
        directions = np.column_stack(
            (
                np.cos(np.radians(self.direction_deg)),
                np.sin(np.radians(self.direction_deg)),
            )
        )
        front = np.einsum("ij,ij->i", vectors, directions) >= -1e-9
        return (self.source_type == 0) | front
    ...
    def predicted_labels(self, position: tuple[float, float]) -> NDArray[np.int64]:
        ...
        front = self._front_mask(position)
        received = self.exists & front & (distances <= self.radii_m+1e-9)
    ...
    def update(self, position: tuple[float,float], result: str, svd_deg: float|None=None) -> None:
        ...
        distances=self._distances(position); front=self._front_mask(position)
        received=self.exists & front & (distances<=self.radii_m+1e-9)
    ...
    def adaptive_refine(self, step_m: float, maximum_particles: int) -> None:
        ...
        points=[]; radii=[]; weights=[]; types=[]; directions=[]
        for index in order:
            ...
            types.extend([int(self.source_type[index])]*len(children))
            directions.extend([float(self.direction_deg[index])]*len(children))
        ...
        self.source_type=np.concatenate((self.source_type,np.asarray(types,dtype=np.int8)))
        self.direction_deg=np.concatenate((self.direction_deg,np.asarray(directions)))
        ...
        if len(self.weights)>maximum_particles:
            ...
            self.exists,self.points,self.radii_m,self.source_type,self.direction_deg,self.weights,self.resolution_m,self.active=(array[keep] for array in (self.exists,self.points,self.radii_m,self.source_type,self.direction_deg,self.weights,self.resolution_m,self.active))
        self.weights/=np.sum(self.weights)
