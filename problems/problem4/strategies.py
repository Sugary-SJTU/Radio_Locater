...
from problems.problem4.coverage import (
    PolygonPlan,
...
from problems.problem4.shared import (
    PlannedAction,
    problem4Executor,
    problem4State,
...
def _ordered_channels(state: problem4State, channels: list[int]) -> list[int]:
...
class Problem4FastStrategy:
    """7点基础覆盖 + 未解决频道外侧补测 + 共享定位清除。"""
    ...
    def run(self, executor: problem4Executor) -> StrategyRunResult:
        state = executor.state
        selected_plan, plans = search_polygon_plans(self.settings)
        base_route = list(selected_plan.points)
        remaining = optimize_remaining_route(state.position, base_route)
        while remaining:
        ...
        # 仍未解决的频道，用外侧6点做定向补测。
        outer = _outer_probe_points()
        for point in optimize_remaining_route(state.position, outer):
...
class BeliefMPCStrategy:
    ...
    def _action_time(self, state: problem4State, action: PlannedAction) -> float:
    ...
    def _generate_candidates(
        ...
        for channel in self.settings.channels:
            ...
            if representative in state.channel_states[channel].measured_positions:
                continue
        ...
        for channel, track in state.tracks.items():
            if track.status == "localized" and track.clear_circle is not None:
                ...
                for point, reason in localization_candidate_points(
                    ...
                    if point in state.channel_states[channel].measured_positions:
                        continue
                    gain = state.beliefs[channel].information_gain_bits(point)
                    mandatory_candidates.append(
    ...
    def _remaining_bounds(
        self, state: problem4State, remaining_route: list[tuple[float, float]]
    ...
    def _fallback_required(
        self, state: problem4State, candidates: list[PlannedAction], has_safe_route: bool = True
    ) -> bool:
    ...
    def _coverage_gain(self, state: problem4State, position: tuple[float,float], channels: tuple[int,...]) -> float:
    ...
    def _generate_candidates(self, state: problem4State, remaining_route: list[tuple[float,float]]) -> list[PlannedAction]:
        ...
        for action in unique.values():
            ...
            movement_time = math_distance(state.position, action.position) / ROBOT_SPEED_MPS
    ...
    def _select_action(self, state: problem4State, remaining_route: list[tuple[float,float]], candidates: list[PlannedAction]) -> PlannedAction:
