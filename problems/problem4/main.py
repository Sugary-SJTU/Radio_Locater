...
def load_settings(path: Path | None, overrides: dict[str, Any]) -> Problem4Settings:
    ...
    allowed = {item.name for item in fields(Problem4Settings)}
    ...
    settings = Problem4Settings(**values)
    _validate_settings(settings)
    return settings


def _validate_settings(settings: Problem4Settings) -> None:
    """在连接模拟器前检查会改变动作含义的配置。"""
...
def run_once(
    *, strategy_name: str, host: str, port: int, robot_id: str,
    timeout_s: float, settings: Problem4Settings, run_index: int,
    ...
    executor = problem4Executor(client, state, logger, strategy_name)
    if strategy_name == FAST_STRATEGY:
        strategy = Problem4FastStrategy(settings)
    elif strategy_name == ROBUST_STRATEGY:
        strategy = RobustPolygonRollingStrategy(settings)
    else:
    ...
    try:
        ...
        executor.exit()
        from problems.problem4.plotting import plot_run_replay
...
__all__ = ["format_run_brief", "load_settings", "run_once", "run_problem4"]
