...
def main() -> None:
    ...
    problem3_parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "config" / "problem3.yaml",
    ...
    problem3_parser.add_argument("--candidate-top-k", dest="candidate_top_k", type=int)
    problem3_parser.add_argument("--horizon", dest="planning_horizon", type=int)
    problem3_parser.add_argument("--beam-width", dest="beam_width", type=int)
    problem3_parser.add_argument("--coarse-grid-size", dest="coarse_grid_size", type=float)
    problem3_parser.add_argument("--fine-grid-size", dest="fine_grid_size", type=float)
    problem3_parser.add_argument("--radius-grid-size", dest="radius_grid_size", type=float)
    problem3_parser.add_argument("--prune-threshold", dest="probability_prune_threshold", type=float)
    problem3_parser.add_argument("--existence-threshold", dest="existence_threshold", type=float)
    problem3_parser.add_argument("--replan-margin", dest="replan_time_margin", type=float)
    problem3_parser.add_argument("--clear-probability", dest="clear_probability", type=float)
    problem3_parser.add_argument("--ablation", dest="ablation_variant", choices=("A", "B", "C", "D", "E"))
    ...
    problem4_parser = subparsers.add_parser(
        "problem4", help="连接附件模拟器运行问题4混合干扰源搜索、定位与清除策略"
    )
    problem4_parser.add_argument(
        "--strategy",
        choices=("robust_polygon_rolling", "belief_mpc", "problem4_fast"),
        required=True,
        help="问题4策略",
    )
    ...
    problem4_parser.add_argument("--config", type=Path, default=PROJECT_ROOT / "config" / "problem4.yaml")
    problem4_parser.add_argument("--seed", type=int)
    problem4_parser.add_argument("--runs", type=int, choices=(1, 3), default=1)
    problem4_parser.add_argument("--formal", action="store_true")
    problem4_parser.add_argument("--case-code", action="append", default=[])
    problem4_parser.add_argument("--official-log", action="append", type=Path, default=[])
    problem4_parser.add_argument("--truth-file", action="append", type=Path, default=[])
    problem4_parser.add_argument("--next-run-wait", type=float, default=0.0)
    ...
    if arguments.problem == "problem1":
        ...
        override_names = (
            ...
            "particle_count_per_channel", "candidate_action_limit", "candidate_top_k",
            "planning_horizon", "beam_width", "coarse_grid_size", "fine_grid_size",
            "radius_grid_size", "probability_prune_threshold", "existence_threshold",
            "replan_time_margin", "clear_probability", "ablation_variant",
            "p0", "g0", "polygon_sides", "polygon_radius_m",
        ...
        for summary in summaries:
            print(format_run_brief(summary))
        print("\n完整JSON汇总：")
        print(json.dumps(summaries, ensure_ascii=False, indent=2))
    elif arguments.problem == "problem4":
        from problems.problem4.main import (
            format_run_brief,
            load_settings,
            run_problem4,
        )
